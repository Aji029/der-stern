/*
  # Add Sammelrechnungen (Collective Invoices) Feature
...
ADD COLUMN IF NOT EXISTS sammelrechnung_id UUID REFERENCES public.sammelrechnungen(id);

-- Create indexes
CREATE INDEX IF NOT EXISTS idx_sammelrechnungen_customer_id ON public.sammelrechnungen(customer_id);
CREATE INDEX IF NOT EXISTS idx_sammelrechnungen_user_id ON public.sammelrechnungen(user_id);
CREATE INDEX IF NOT EXISTS idx_orders_sammelrechnung_id ON public.orders(sammelrechnung_id);
...
BEGIN
  ...
  WITH order_totals AS (
    SELECT 
      ...
      SUM(CASE 
        WHEN o.mwst = 'A' THEN total_amount / (1 + 0.07)
        WHEN o.mwst = 'B' THEN total_amount / (1 + 0.19)
        ELSE total_amount
      END) as total_netto,
      SUM(CASE 
        WHEN o.mwst = 'A' THEN total_amount - (total_amount / (1 + 0.07))
        WHEN o.mwst = 'B' THEN total_amount - (total_amount / (1 + 0.19))
        ELSE 0
      END) as total_vat
